...
function looksQAArray(x: any): x is QA[] {
  return Array.isArray(x) && x.every(
    (i) => i && typeof i === "object" && typeof i.q === "string" && typeof i.a === "string"
  );
}

function deepFindFirstArray(obj: any): any[] | null {
  const stack = [obj];
  while (stack.length) {
    const v = stack.pop();
    if (Array.isArray(v)) return v;
    if (v && typeof v === "object") for (const k of Object.keys(v)) stack.push(v[k]);
  }
  return null;
}
...
function extractCardsServer(content: string): QA[] {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      if (arr && looksQAArray(arr)) return arr;
    }
  } catch {}
  ...
  // 3) Try top-level array substring
  ...
  // 4) Extract object blocks individually (tolerate truncated output)
  const objs: QA[] = [];
  ...
  for (const c of candidates) {
    try {
      ...
      if (o && typeof o.q === "string" && typeof o.a === "string") objs.push({ q: o.q, a: o.a });
    } catch {}
  }
